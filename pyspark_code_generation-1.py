...
# Load the CSV data
df = pd.read_csv('shema_grouped.csv')

# Filter columns based on data type
filtered_df = df[df['Data Type'] == 'array<struct>']
if not filtered_df.empty:
    array_columns = ast.literal_eval(filtered_df['Column Name'].iloc[0])
else:
    array_columns = []

filtered_df1 = df[df['Data Type'] == 'struct']
if not filtered_df1.empty:
    struct_columns = ast.literal_eval(filtered_df1['Column Name'].iloc[0])
else:
    struct_columns = []
...
def write_operations_to_file(input_strings, filename, present):
    ...
    def process_input_string(input_string):
        ...
        for i, name in enumerate(result):
            if i == 0:
                df_name = f"df_{name}"
                data += f"    {df_name} = {base_df_name}.select('_id', explode_outer('{name}').alias('{name}'))\n"
            else:
                previous_name = result[i-1]
                previous_df_name = f"df_{previous_name.replace('.', '_')}"
                df_name = f"df_{name.replace('.', '_')}"
                data += f"    {df_name} = {previous_df_name}.select('_id', explode_outer('{name}').alias('{name.replace('.', '_')}'))\n"
        ...
        if additional_columns:
            ...
            data += f"    {last_df_name}_final = {last_df_name}.select('_id', '{last_column_name}.*', '{columns_to_add}')\n"
        else:
            data += f"    {last_df_name}_final = {last_df_name}.select('_id', '{last_column_name}.*')\n"
...
filename = "dataframe_operations.py"
